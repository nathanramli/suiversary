...
const submitProof = async (coin: Coin, gas: Coin) => {
    try {
        ...
        txb.setGasPayment([{ objectId: gas.objectId, version: gas.version, digest: gas.digest  }])
        ...
        txb.moveCall(`${publishResult.packageId}::suiversary::mint_pow`,
            [
...
const observeObjects = async () => {
    try {
        ...
        let cursor = null;
        do {
            const resp = await suiKit.provider().getCoins({ owner: suiKit.currentAddress(), coinType: SUI_TYPE_ARG, cursor: cursor, limit: 100 });
            resp.data.map((data) => {
                coins.push({
